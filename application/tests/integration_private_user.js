...
describe('REST API', () => {
  ...
  context('when trying to test the secured user stuff', () => {
    ...
    it('it should reply UNAUTHORIZED for a not existing user', () => {
      let opt = Object.assign({}, options);
      opt.url += '10' + '/profile';
      opt.headers['----jwt----'] = jwtHeader;
      return server.inject(opt).then((response) => {
    ...
    it('it should reply UNAUTHORIZED in case the JWT is missing', () => {
      let opt = Object.assign({}, options);
      opt.url += userid + '/profile';
      console.log(opt); //TODO JWT is filled up and I HATE JAVASCRIPT!
      return server.inject(opt).then((response) => {
        response.should.be.an('object').and.contain.keys('statusCode', 'payload');
        response.statusCode.should.equal(401);
        response.payload.should.be.a('string');
